...
ifeq ($(SWIARCH),x64-win64)
	RINSIDEQ=$(shell Rscript -e "cat(shQuote(dirname(paste(system.file(package='RInside'), grep('x64/libRInside.$(SOEXT)', list.files(path=system.file(package='RInside'), recursive=TRUE), value=TRUE), sep='/'))))")
